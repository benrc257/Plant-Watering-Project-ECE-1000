...
def readhumid(): # Calculates humidity from sensor
    ...
    humidity = int(100-(100*((read-water)/(air-water)))) # Calculates the humidity % using the values previously defined (converts to integer)
    if (humidity > 100): # If the percentage goes above 100%, caps it at 100%
        humidity = 100
    print("Humidity:", humidity, "Sensitivity:", sensitivity[0]) # Prints the humidity and sensitivity to the terminal
